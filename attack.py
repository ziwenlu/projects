...
def prep(im):
    print(im.shape)
    # img=cv2.imshow('img',im)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    if len(im.shape) == 3:
        return np.transpose(im, [2, 1, 0]).reshape((1, 3, 112, 112)) * 2 - 1
    elif len(im.shape) == 4:
...
def main(args):
    ...
    logo = tf.compat.v1.placeholder(tf.float32, shape=[None, 400, 900, 3], name='logo_input')
    ...
    while True:
        ...
        step += 1
        if step % 20 == 0:
            print('Stage:', stage, 'Step:', step, 'Av. time:')#, round((time() - t) / step, 2), 'Loss:', round(ls[-1], 2),
                  #'Coef:', regr_coef)
