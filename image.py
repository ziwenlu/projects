...
def prep(im):
    print(im.shape)
    if len(im.shape)==3:
        return np.transpose(im,[2,0,1]).reshape((1,3,400,900))*2-1
    elif len(im.shape)==4:
        return np.transpose(im,[0,3,1,2]).reshape((im.shape[0],3,400,900))*2-1
...
def main(args):
    ...
    grads_input = tf.placeholder(tf.float32, shape=[None, 400, 900, 3], name='grads_input')
    ...
    if args.anchor_face != None:
        ...
        anch_im=cv2.imread("lzwhat_aligned.png")
        #anch_im = rescale(io.imread(args.image) ,, order=5)
        fdict2[image_input] = prep(anch_im)
        fdict2[orig_emb] = sess.run(embedding, feed_dict=fdict2)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--param', type=float, default=0.0013,
                        help='Parabola rate for the off-plane parabolic transformation')
